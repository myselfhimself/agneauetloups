...
void preIA(t_game *game,int depth)
{
    ...
    t0 = time(NULL);
	/** si agneau :\n
	* - si possible et pas trop �oign�lanc�l'IA sur l'emplacement en haut �gauche\n
	* - si possible et pas trop �oign�lanc�l'IA sur l'emplacement en haut �droite\n
	* - si possible et pas trop �oign�lanc�l'IA sur l'emplacement en bas �gauche\n
	* - si possible et pas trop �oign�lanc�l'IA sur l'emplacement en bas �droite\n
	*/
	if(player == LAMB)
	{
	    ...
	    /** - si la somme des probabilit� est jug� trop petite pour avoir de l'importance\n
		* -# avancer al�toirement, sur le devant si possible\n
		*/
		if(abs(sum)< 4000)
	    {
		...
		/** - si la somme des probabilit� est tr� grande alors la solution est �idente et proche\n
		* -# soit elle est en haut �gauche => essai d'y aller\n
		* -# soit elle est en haut �droite => essai d'y aller\n
		*/
		...
		/** - sinon allez vers la plus grande des possibilit�\n
		*/
        {
    ...
    else
    {
		...
		* -# trouver le prochain loup �d�lacer\n
		* -# le d�lacer\n
		*/
        ...
        {
            /** - sinon se d�lacer vers la plus basse probabilit�n
		    */
            for(i=1;i<6;i++)
            {
...
int IA(int player,int depth,int lx,int ly,int w1x,int w1y,int w2x,int w2y,int w3x,int w3y,int w4x,int w4y,int w5x,int w5y)
{
	...
	/** - si le jeu est termin�n
	* -# si l'agneau a gagn�retourner un coefficient d'issue positif\n
	* -# si les loups ont gagn� retourner un coefficient d'issue n�atif\n
	*/
	game_result = game_is_over(pawn);	
	switch(game_result)
	{//si les agneaux on gagn�		
		case 1:	
			count = importance(depth);
			break;
		//si les loups on gagn�
		case -1:
			count = (-1 * importance(depth));
			break;
		default:
		/** - si la profondeur maximale a ��atteinte retourner z�o\n
		*/
		if(depth == 0) count = 0;
		/** - si le joueur est agneau : rapeller l'IA r�ursivement sur les 4 positions possibles\n
		*/
		else if(player == LAMB)
		{
		if(possible_and_free(lx+1,ly+1,pawn) && isnt_too_far(lx+1,ly+1,pawn))
		...
		/** - si le joueur est loup : rapeller l'IA r�ursivement sur les 10 positions possibles\n
		*/
		else
		{
			for(k=1;k<6;k++)
